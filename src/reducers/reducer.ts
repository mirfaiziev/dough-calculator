...
// if hydration is changed we assume the flour will be the same.
const handleHydrationChanges = (state: AppStateInterface, hydration: number): AppStateInterface=> {
    ...
    return {
        ...
        flourWeight,
        waterWeight,
        totalDoughWeight: flourWeight+waterWeight
    }
}
// hydration we keep the same, because it's what we are interested in, but flour and total dough will be changed
const handleWaterWeightChanged = (state: AppStateInterface, waterWeight: number): AppStateInterface=> {
    ...
    const flourWeight =  (waterWeight / hydration) * 100
...
const reducer = (state: AppStateInterface, action: AppActionsInterface) => {
    switch (action.type) {
        case ActionType.ChangeHydration:
            return handleHydrationChanges(state, action.payload)
